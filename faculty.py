...
class student:
    ...
    def __init__(self, root):
        ...
        tree = ttk.Treeview(root, column=("FACULTY_ID", "FIRSTNAME","LASTNAME", "EMAIL", "ADDRESS", "GENDER", "PHONE NO."), show='headings')
    
        tree.column("FACULTY_ID", width=110, anchor='center')
        ...
        tree.column("GENDER", width=100, anchor='cente')
        tree.column("PHONE NO.", width=120, anchor='center')

        tree.heading("#1", text="FACULTY_ID")
        ...
        def OnDoubleClick(self):
            curItem = tree.focus()
            faculty_id = tree.item(curItem)['values'][0]
            print(faculty_id)
        ...
        def DisplayData():
            ...
            cur = conn.cursor()
            cur.execute("SELECT faculty_id,firstname, lastname, email, address, gender, phone FROM faculty")
        ...
        def addData():
            ...
            if(len(firstname.get())==0):
                ...
                email_exist_for_faculty = LMS_verification.email_exist_for_faculty(email.get())
                
                phone_check = LMS_verification.phoneisValid(phoneno.get())
                phone_exist_for_faculty = LMS_verification.phone_exist_for_faculty(phoneno.get())
                ...
                if (email_exist_for_faculty==0 and email_check==0 and phone_check!=None and phone_exist_for_faculty==0):
                    if(len(firstname.get())!=0):
                        LibBksDatabase.addFaculty(firstname.get(),lastname.get(),email.get(),address.get(),gender ,dept_id, phoneno.get())
        ...
        def UpdateData(faculty_id):
            conn = sqlite3.connect("libbooks.db")
            cur = conn.cursor()
            cur.execute("SELECT firstname, lastname, email, address, gender, phone, dept_id FROM faculty WHERE faculty_id=?", [faculty_id])
            rows = cur.fetchall()
            conn.close()
        ...
        def UpdateFacultyData():
            ...
            email_check = LMS_verification.email_check(email.get())
            email_exist_for_update_faculty = LMS_verification.email_exist_for_update_faculty(email.get(), faculty_id)
            ...
            if (len(phoneno.get())!=10 ):
                ...
                phone_exist_for_update_faculty = LMS_verification.phone_exist_for_update_faculty(phoneno.get(), faculty_id)

                if (email_exist_for_update_faculty!=1 and phone_exist_for_update_faculty!=1):
                    conn = sqlite3.connect("libbooks.db")
                    cur = conn.cursor()
                    cur.execute("UPDATE faculty SET firstname=?, lastname=?, email=?, address=?, gender=?, dept_id=?, phone=? where faculty_id=?", (firstname.get(),lastname.get(),email.get(),address.get(),gender ,dept_id, phoneno.get(), faculty_id))
                    conn.commit()
                    conn.close()
        ...
        self.studenttitle = Label(DataFrameLEFT, font=('arial',18,'bold'), text="Faculty Registration", bg = "Cadet blue")
        ...
        self.btnUpdateData = Button(ButtonFrame, text='Update Data', font=('arial', 12, 'bold'), height=2, width=13, bd = 4, command=UpdateFacultyData)
