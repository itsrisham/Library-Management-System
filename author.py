...
class student:
    ...
    def __init__(self, root):
        ...
        tree = ttk.Treeview(root, column=("Author_ID", "Author"), show='headings')
    
        tree.column("Author_ID", width=150)
        tree.column("Author", width=170)
        

        tree.heading("#1", text="Author_ID")
        ...
        def OnDoubleClick(self):
            curItem = tree.focus()
            author_id = tree.item(curItem)['values'][0]
        ...
        def addData():
            ...
            check_author_exist = LMS_verification.check_author_exist(author.get())
            

            if(len(author.get())<=1):
                authormsg = tkinter.messagebox.showinfo("Library Management System", "Please Enter Author Name!")
            
            if (check_author_exist!=1):
                LibBksDatabase.addAuthor(author.get())
                tkinter.messagebox.showinfo("Library Management System", "Inserted Data Successfully!")
                DisplayData()
            

        def UpdateData(author_id):
            conn = sqlite3.connect("libbooks.db")
            cur = conn.cursor()
        ...
        def UpdatDepartmentData():
            ...
            check_author_exist_for_update = LMS_verification.check_author_exist_for_update(author_id, author.get())
           
           
            if(len(author.get())<=1 ):
                authormsg = tkinter.messagebox.showinfo("Library Management System", "Please Enter Author Name!")
           
            if (check_author_exist_for_update!=1):
                conn = sqlite3.connect("libbooks.db")
                cur = conn.cursor()
                cur.execute("UPDATE author SET name=? where author_id=?", (author.get(), author_id) )
                conn.commit()
                conn.close()
        ...
        self.authorTitle = Label(DataFrameLEFT, font=('arial',18,'bold'), text="Add Author", bg = "Cadet blue")
        self.authorTitle.grid(row =0, column=0, columnspan=2)
        ...
        self.txtAuthor = Entry(DataFrameLEFT, font=('arial',12,'bold'),textvariable=author, width=23)
